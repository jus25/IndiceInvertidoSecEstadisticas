...
public class InvertedIndex {
    ...
    private final String DFileLinesName = "FilesLinesContent"; // Nombre fichero donde guardar las lineas de los
    // ficheros indexados
    private final String DFilesIdsName = "FilesIds"; // Nombre fichero donde guardar las identificadores de los ficheros
    // indexados
    private final String DDefaultIndexDir = "./Index/"; // Directorio por defecto donde se guarda el indice invertido.

    private final float DMatchingPercentage = 0.80f; // Porcentaje mínimo de matching entre el texto original y la
    // consulta (80%)
    private final float DNearlyMatchingPercentage = 0.60f; // Porcentaje mínimo de matching entre el texto original y la
    // consulta (80%)
    ...
    private RandomAccessFile randomInputFile; // Fichero random para acceder al texto original con mayor porcentaje de
    // matching.
    ...
    // Hash Map que implementa el Índice Invertido: key=word,
    // value=Locations(Listof(file,line)).
    ...
    public long getTotalWords(Map hash) {
        return (hash.size());
    }
    ...
    // Si una palabra aparece varias veces en un linea de texto, solo se cuenta una
    // vez.
    public long getTotalLocations(Map hash) {
        ...
        while (keyIterator.hasNext()) {
            ...
            // System.out.printf(ANSI_BLUE+"[%d-%d] %s -->
        ...
        if (content != null) {
            for (int i = 0; i < content.length; i++) {
                if (content[i].isDirectory()) {
                    ...
                    // Si es un fichero de texto, añadirlo a la lista para su posterior
                    // procesamiento.
    ...
    // Método para Contruir el Indice Invertido a partir de todos los ficheros de
    // texto encontrados
    // en el árbol de directorios.
    // En el Indice invertido se almacena los ids lógicos de los ficheros
    // procesados. La correspondencia
    // idFichero->RutaNombreFichero se guarda en el hash Files
    public void buidIndexFiles(int fileId) {
        // Procesar cada uno de los ficheros de texto.
        for (File file : FilesList) {
            Files.put(fileId, file.getAbsolutePath()); // Añadir entra en el hash de traducción de Id a ruta+nombre
            // fichero
    ...
    // invertido todos los ficheros
    // de la lista.
    // 1. Se lee cada un de las líneas de texto del fichero, se eliminan los
    // caracteres especiales y se
    // divide el contenido de la linea en palabras.
    // 2. Para cada palabra, se accede a su entrada en la tabla de hash del indice
    // invertido y se comprueba
    // si existe la clave (palabra). Si no existe la clave, se crea una nueva lista,
    // añadiendo la localización
    // de la palabra (id_fichero + línea), y está se añade al hashmap del indice
    // asociandao con la palabra
    // encontrada como clave. Si existe la clave, se añade una nueva entrada en su
    // lista de localizaciones.
    // También se genera una hasp con todas las líneas de los ficheros indexados
    ...
    // Método para salvar el indice invertido en el directorio pasado como
    // parámetro.
    // Se salva:
    ...
    public void saveInvertedIndex1(String outputDirectory, List<Map.Entry<String, HashSet<Location>>> listHash, long begin, long end, int numFile) {

        try {
            File KeyFile = new File(outputDirectory + "/" + DIndexFilePrefix + String.format("%03d", numFile));
            FileWriter fw = new FileWriter(KeyFile);
            BufferedWriter bw = new BufferedWriter(fw);

            // Recorremos las claves correspondientes a este fichero
            while (begin < end) {
                Map.Entry<String, HashSet<Location>> entrada = listHash.get((int) begin);
                saveIndexKey1(entrada, bw);
                begin++;
            }
            bw.close(); // Cerramos el fichero.

        } catch (IOException e) {
            System.err.println("Error creating Index file " + outputDirectory + "/IndexFile" + begin);
            e.printStackTrace();
            System.exit(-1);
        }
    ...
    public void saveIndexKey1(Map.Entry<String, HashSet<Location>> entrada, BufferedWriter bw) {

        try {
            // Creamos un string con todos los offsets separados por una coma.
            // String joined1 = StringUtils.join(locations, ";");
            String joined = String.join(";", entrada.getValue().toString());
    ...
    // Recibe la ruta del directorio en donde se van a guardar los ficheros del
    // indice.
    ...
    public void loadInvertedIndex1(File file) {
    ...
    public void loadInvertedIndex(String inputDirectory) {
        ...
        for (File file : listOfFiles) {
            if (file.isFile()) {
                // System.out.println("Processing file " + folder.getPath() + "/" +
                // file.getName()+" -> ");
                try {
                    FileReader input = new FileReader(file);
                    BufferedReader bufRead = new BufferedReader(input);
    ...
    public void loadFilesLines(String inputDirectory) {
        try {
            ...
            try {
                // Leemos fichero línea a linea (clave a clave)
                while ((keyLine = bufRead.readLine()) != null) {
                    // Descomponemos la línea leída en su clave (Location) y la linea de texto
                    // correspondiente
                    String[] fields = keyLine.split("\t");
    ...
    public Map<Location, Integer> queryConc (String word){
        
        String queryResult = null;
        Map<Location, Integer> queryMatching = new TreeMap<Location, Integer>();
        //Instant start = Instant.now();
        
        word = word.toLowerCase();

        if (Indexing.Verbose)
                System.out.printf("Word %s matching: ", word);
            // Procesar las distintas localizaciones de esta palabra
        if (Hash.get(word) != null){
            
            for (Location loc : Hash.get(word)) {
                    // Si no existe esta localización en la tabla de coincidencias, entonces la
                    // añadimos con valor inicial a 1.
                    Integer value = queryMatching.putIfAbsent(loc, 1);
                if (value != null) {
                        // Si existe, incrementamos el número de coincidencias para esta localización.
                        queryMatching.put(loc, value + 1);
                }
                if (Indexing.Verbose)
    ...
    // 4. Recorremos la tabla de coincidencia y mostramos las coincidencias en
    // función del porcentaje de matching.
    public void query(String queryString) {
        ...
        // Recorremos la tabla de coincidencia y mostramos las líneas en donde aparezca
        // más de un % de las palabras de la query.
        for (Map.Entry<Location, Integer> matching : queryMatchings.entrySet()) {
            ...
            if ((matching.getValue() / (float) querySize) == 1.0)
                System.out.printf(
                        ANSI_GREEN_YELLOW_UNDER + "%.2f%% Full Matching found in line %d of file %s: %s.\n"
                                + ANSI_RESET,
                        (matching.getValue() / (float) querySize) * 100.0, location.getLine(), location.getFileId(),
                        getIndexFilesLine(location));
    ...
    public void stats( Map<Location, Integer> queryMatchings, int querySize){
